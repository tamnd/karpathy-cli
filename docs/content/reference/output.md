...
describes what users get. Pick a format with `-o`, or let karpathy choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
karpathy <command> -o table   # aligned columns for reading
karpathy <command> -o jsonl   # one JSON object per line, for piping
karpathy <command> -o json    # a single JSON array
karpathy <command> -o csv     # spreadsheet friendly
karpathy <command> -o tsv     # tab-separated
karpathy <command> -o url     # just the URL column
karpathy <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
karpathy <command> --fields id,title,url
```
...
```bash
karpathy <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
karpathy <command>            # a table, because this is a terminal
karpathy <command> | wc -l    # JSONL, because this is a pipe
```
